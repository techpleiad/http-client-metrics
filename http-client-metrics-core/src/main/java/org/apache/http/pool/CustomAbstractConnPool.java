        ...
        if (pool == null) {
            pool = new RouteSpecificPool<T, C, E>(route) {
                ...
                protected E createEntry(final C conn) {
                    return CustomAbstractConnPool.this.createEntry(route, conn);
                }

            };
            final String routeName = route.toString();
            Gauge.builder("http_connections_" + routeName, pool, RouteSpecificPool::getAvailableCount).tag("type", "available")
                    .description("Available http connections " + routeName).register(this.meterRegistry);
            Gauge.builder("http_connections_" + routeName, pool, RouteSpecificPool::getLeasedCount).tag("type", "leased")
                    .description("Leased http connections " + routeName).register(this.meterRegistry);
            Gauge.builder("http_connections_" + routeName, pool, RouteSpecificPool::getPendingCount).tag("type", "pending")
                    .description("Leased http connections " + routeName).register(this.meterRegistry);
            Gauge.builder("http_connections_" + routeName, pool, RouteSpecificPool::getAllocatedCount).tag("type", "allocated")
                    .description("Allocated http connections " + routeName).register(this.meterRegistry);
            Gauge.builder("http_connections_" + routeName, () -> Integer.valueOf(this.getMaxPerRoute(route))).tag("type", "total")
                    .description("Total http connections " + routeName).register(this.meterRegistry);
